...
const ServiceList = () => {
    ...
    useEffect(() => {
        ...
        if (statusFilter !== "all") {
            result = result.filter(service =>
                statusFilter === "active" ? service.active === 1 : service.active === 0
            );
        }
        ...
        setPage(1);
    }, [searchTerm, statusFilter, orderBy, order, services]);
    ...
    const handleDelete = (service) => {
        const action = service.active === 1 ? "desactivar" : "activar";
        Swal.fire({
            title: `¿${service.active === 1 ? 'Desactivar' : 'Activar'} servicio?`,
            html: `
                <div style="text-align: left">
                    <p><strong>${service.name}</strong></p>
                    <p>Descripción: ${service.description || 'Sin descripción'}</p>
                    <p>Estado actual: ${service.active === 1 ? 'Activo' : 'Inactivo'}</p>
                </div>
            `,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: service.active === 1 ? '#d33' : 'var(--success)',
            cancelButtonColor: 'var(--primary)',
            confirmButtonText: service.active === 1 ? 'Sí, desactivar' : 'Sí, activar',
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    Swal.fire(
                        '¡Completado!',
                        `El servicio ha sido ${service.active === 1 ? 'activado' : 'desactivado'} correctamente.`,
                        'success'
                    );
                    fetchServices();
                } catch (error) {
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
                    <Box>
                        ...
                        <Typography variant="body2" color="text.secondary">
                            Gestiona todos los servicios incluidos: {filteredServices.length} registros encontrados
                            {services.length > 0 && (
                                <span style={{ marginLeft: '8px' }}>
                                    | Activos: {services.filter(s => s.active === 1).length}
                                </span>
                            )}
                        </Typography>
                    </Box>
                                ...
                                paginatedServices.map((service) => (
                                    <TableRow
                                        ...
                                        sx={{
                                            ...
                                            opacity: service.active === 1 ? 1 : 0.6
                                        }}
                                    >
                                        ...
                                        <TableCell>
                                            <Tooltip title={service.description || 'Sin descripción'} arrow>
                                        ...
                                        <TableCell>
                                            <Chip
                                                icon={service.active === 1 ? <CheckIcon /> : <CancelIcon />}
                                                label={service.active === 1 ? "Activo" : "Inactivo"}
                                                color={service.active === 1 ? "success" : "default"}
                                                size="small"
                                                variant="outlined"
                                            />
                                        </TableCell>
                                        <TableCell align="right">
                                            ...
                                            <Button
                                                variant="outlined"
                                                color={service.active === 1 ? "error" : "success"}
                                                size="small"
