...
const CategoryList = () => {
    ...
    useEffect(() => {
        ...
        setPage(1);
    }, [searchTerm, statusFilter, orderBy, order, categories]);
    ...
    const handleDelete = (category) => {
        const action = category.active === 1 ? "desactivar" : "activar";
        Swal.fire({
            title: `¿${category.active === 1 ? 'Desactivar' : 'Activar'} categoría?`,
            html: `
                <div style="text-align: left">
                    <p><strong>${category.name}</strong></p>
                    <p>Descripción: ${category.description || 'Sin descripción'}</p>
                    <p>Estado actual: ${category.active === 1 ? 'Activo' : 'Inactivo'}</p>
                </div>
            `,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: category.active === 1 ? '#d33' : 'var(--success)',
            cancelButtonColor: 'var(--primary)',
            confirmButtonText: category.active === 1 ? 'Sí, desactivar' : 'Sí, activar',
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    Swal.fire(
                        '¡Completado!',
                        `La categoría ha sido ${category.active === 1 ? 'activada' : 'desactivada'} correctamente.`,
                        'success'
                    );
                    fetchCategories();
                } catch (error) {
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
                    <Box>
                        ...
                        <Typography variant="body2" color="text.secondary">
                            Gestiona todas las categorías: {filteredCategories.length} registros encontrados
                            {categories.length > 0 && (
                                <span style={{ marginLeft: '8px' }}>
                                    | Activas: {categories.filter(c => c.active === 1).length}
                                </span>
                            )}
                        </Typography>
                    </Box>
                                ...
                                paginatedCategories.map((category) => (
                                    <TableRow
                                        ...
                                        sx={{
                                            ...
                                            opacity: category.active === 1 ? 1 : 0.6
                                        }}
                                    >
                                        ...
                                        <TableCell>
                                            <Tooltip title={category.description || 'Sin descripción'} arrow>
                                        ...
                                        <TableCell>
                                            <Chip
                                                icon={category.active === 1 ? <CheckIcon /> : <CancelIcon />}
                                                label={category.active === 1 ? "Activo" : "Inactivo"}
                                        ...
                                        <TableCell align="right">
                                            ...
                                            <Button
                                                variant="outlined"
                                                color={category.active === 1 ? "error" : "success"}
                                                size="small"
